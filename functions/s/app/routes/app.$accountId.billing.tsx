...
export const loader = ReactRouter.routeEffect(({ context }) =>
  Effect.fromNullable(context.get(ReactRouter.appLoadContext).account).pipe(Effect.map((account) => ({ account })))
)
...
export default function RouteComponent({ loaderData: { account } }: Route.ComponentProps) {
  return (
    <div className="space-y-6">
      ...
      <Card className="w-full max-w-md">
        ...
        <CardContent>
          <p className="text-muted-foreground text-sm">
            Current Plan: <span className="text-foreground font-medium">{account.planName || 'Free'}</span>
          </p>
        </CardContent>
      ...
      <pre className="bg-muted mt-4 overflow-x-auto rounded-md p-4 text-sm">{JSON.stringify({ account }, null, 2)}</pre>
    </div>
  )
}
